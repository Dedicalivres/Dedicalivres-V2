...
export function GrimoireVivant({ fallback: _fallback }: GrimoireVivantProps) {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => { e.preventDefault(); st.scroll += e.deltaY * 0.007; };
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (now - st.lastClick < 340) { toggleBook(cx, cy); }
      else { playCrystal(); spawnSpark(cx, cy); spawnLetter(cx, cy); }
      st.lastClick = now;
    };
  ...
  return (
    <div
      ref={rootRef}
      style={{ position: "relative", width: "100%", height: "480px", overflow: "hidden", cursor: "none", background: "transparent" }}
