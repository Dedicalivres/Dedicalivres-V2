...
export function GrimoireVivant({ fallback: _fallback }: GrimoireVivantProps) {
  ...
  // Identifiant unique par instance pour éviter les conflits d'id
  const idRef = useRef(`gv-${Math.random().toString(36).slice(2, 7)}`);
  ...
  const playCrystal = useCallback(async () => {
    try {
      ...
      m.gain.setValueAtTime(0.14, t);
      m.gain.exponentialRampToValueAtTime(0.001, t + 0.5);
      m.connect(ac.destination);
      [1046.5, 1318.5, 1568].forEach((f, i) => {
        const o = ac.createOscillator(); const g = ac.createGain();
        o.type = "sine"; o.frequency.setValueAtTime(f, t);
        g.gain.setValueAtTime([0.9, 0.55, 0.3][i], t);
  ...
  const playBookOpen = useCallback(async () => {
    try {
      ...
      const rvg = ac.createGain(); rvg.gain.setValueAtTime(0.3, t);
      ...
      ([[0, 180, 0.7],[0.06, 280, 0.45],[0.10, 350, 0.3],[0.18, 220, 0.22]] as [number,number,number][])
        .forEach(([dt, fr, vol]) => {
          ...
          f.frequency.setValueAtTime(fr, t); f.Q.setValueAtTime(1.2, t);
          const g = ac.createGain();
          g.gain.setValueAtTime(vol, t + dt);
          g.gain.exponentialRampToValueAtTime(0.001, t + dt + 0.09);
          s.connect(f); f.connect(g); g.connect(dry); g.connect(rv);
          s.start(t + dt); s.stop(t + dt + 0.12);
        });

      ([[0.12,0.28,1200,0.18],[0.28,0.22,1800,0.13],[0.42,0.30,1400,0.10],[0.60,0.20,2000,0.07],[0.76,0.18,1600,0.05]] as [number,number,number,number][])
        .forEach(([dt, dur, fr, vol]) => {
          ...
          s.connect(hp); hp.connect(lp); lp.connect(g); g.connect(dry); g.connect(rv);
          s.start(t + dt); s.stop(t + dt + dur + 0.05);
        });

      [196, 246.9, 293.7, 392, 493.9].forEach((f, i) => {
        const o = ac.createOscillator(); const g = ac.createGain();
        o.type = i < 2 ? "sine" : "triangle"; o.frequency.setValueAtTime(f, t);
        const s2 = t + 0.35 + i * 0.18;
        g.gain.setValueAtTime(0, s2);
        g.gain.linearRampToValueAtTime([0.12, 0.10, 0.08, 0.06, 0.04][i], s2 + 0.4);
        g.gain.exponentialRampToValueAtTime(0.001, s2 + 2.8);
        o.connect(g); g.connect(dry); g.connect(rv); o.start(s2); o.stop(s2 + 3.2);
      });
    } catch {}
  }, [getAC]);
  ...
  const spawnLetter = useCallback((x: number, y: number) => {
    ...
    const dx = (Math.random() - 0.5) * 65;
    const dy = -(20 + Math.random() * 55);
    const r0 = (Math.random() - 0.5) * 12;
  ...
  const spawnSpark = useCallback((x: number, y: number) => {
    ...
    el.width = s * 4; el.height = s * 4;
    el.style.cssText = `position:absolute;pointer-events:none;z-index:90;left:${x - s * 2}px;top:${y - s * 2}px`;
    const c = Math.random() > 0.5
      ? `rgba(212,177,106,${0.7 + Math.random() * 0.3})`
      : `rgba(90,240,160,${0.6 + Math.random() * 0.3})`;
    const ctx = el.getContext("2d")!;
    ctx.fillStyle = c; ctx.strokeStyle = c; ctx.lineWidth = 0.8;
    ...
    if (type === 0) {
      ...
      for (let i = 0; i < pts; i++) {
        const a = i * (Math.PI * 2 / pts) + (Math.random() - 0.5) * 0.7;
      ...
      ctx.lineTo(cx2 + Math.cos(a) * sz * 1.4, cy2 + Math.sin(a) * sz * 1.4); ctx.stroke();
    } else if (type === 2) {
      ctx.beginPath();
      ctx.moveTo(cx2 - sz, cy2); ctx.lineTo(cx2 + sz, cy2);
      ctx.moveTo(cx2, cy2 - sz); ctx.lineTo(cx2, cy2 + sz); ctx.stroke();
    } else if (type === 3) {
      for (let i = 0; i < 3; i++) {
        const a = i * (Math.PI * 2 / 3) + Math.random() * 0.5;
        ctx.beginPath(); ctx.moveTo(cx2, cy2);
        ctx.lineTo(cx2 + Math.cos(a) * sz * 1.5, cy2 + Math.sin(a) * sz * 1.5); ctx.stroke();
      }
    } else {
      ctx.beginPath();
      ctx.arc(cx2, cy2, sz * 0.7, Math.random() * Math.PI, (Math.random() + 1) * Math.PI); ctx.stroke();
    }
    const dx = (Math.random() - 0.5) * 80; const dy = -(20 + Math.random() * 70);
    const r0 = (Math.random() - 0.5) * 20; const r1 = (Math.random() - 0.5) * 60;
  ...
  const startWriting = useCallback(async () => {
    ...
    const fw  = featherRef.current;
    const svg = writingSvgRef.current;
    const root= rootRef.current;
    if (!fw || !svg || !root) return;
    const fwEl = fw;
    fwEl.style.opacity = "1";

    const seq = [
      { rectId: "gv-cr-b", x0: 5, x1: 255, ySvg: 138, dur: 1560, delay: 0,    fullW: 260 },
      { rectId: "gv-cr-d", x0: 5, x1: 255, ySvg: 182, dur: 1920, delay: 1700, fullW: 260 },
    ];

    for (const s of seq) {
      await new Promise<void>(resolve => {
        setTimeout(() => {
          const el = svg.getElementById ? svg.getElementById(s.rectId) as SVGRectElement | null
                                        : document.getElementById(s.rectId) as SVGRectElement | null;
          ...
          function step(now: number) {
            ...
            fwEl.style.left = `${cx}px`; fwEl.style.top = `${cy}px`;
            fwEl.style.transform = `translate(-50%,-100%) rotate(${-38 + Math.sin(now * 0.021) * 2.8}deg)`;
  ...
  const toggleBook = useCallback(async (cx: number, cy: number) => {
    ...
    if (st.isOpen) {
      await playBookOpen();
      st.flashActive = true; st.flashStart = performance.now();
      st.flashCx = cx; st.flashCy = cy;
      ...
      const fc = flashCvsRef.current; if (!fc) return;
      const ctx = fc.getContext("2d"); if (ctx) ctx.clearRect(0, 0, fc.width, fc.height);
    }
  ...
  useEffect(() => {
    ...
    // Canvas couverture (positionné en absolu, PAS dans foreignObject)
    const cvs = coverCvsRef.current;
    let c2: CanvasRenderingContext2D | null = null;
    if (cvs) { cvs.width = 260; cvs.height = 340; c2 = cvs.getContext("2d"); }

    // Canvas flash
    const flashCvs = flashCvsRef.current;
    let fc: CanvasRenderingContext2D | null = null;
    function resizeFlash() {
      if (!flashCvs) return;
      const r = root!.getBoundingClientRect();
      flashCvs.width  = Math.max(r.width,  800);
      flashCvs.height = Math.max(r.height, 560);
      fc = flashCvs.getContext("2d");
    }
    resizeFlash();
    ...
    veinsRef.current = Array.from({ length: 14 }, () => ({
      x: Math.random() * 260, y: 0,
      cp1x: Math.random() * 260, cp1y: 60 + Math.random() * 100,
      cp2x: Math.random() * 260, cp2y: 180 + Math.random() * 100,
      ex: Math.random() * 260, ey: 340,
      c: Math.random() > 0.5 ? "rgba(70,210,130," : "rgba(200,165,90,",
      w: 0.3 + Math.random() * 1, ph: Math.random() * Math.PI * 2,
    }));
    nodesRef.current = Array.from({ length: 28 }, () => ({
      x: 10 + Math.random() * 240, y: 10 + Math.random() * 320,
    ...
    function drawCover(t: number) {
      if (!c2) return;
      const c2n = c2; const w = 260; const h = 340;
      c2n.clearRect(0, 0, w, h);
      veinsRef.current.forEach(v => {
        const a = 0.15 + Math.sin(t * 0.5 + v.ph) * 0.13;
        c2n.beginPath(); c2n.moveTo(v.x, v.y);
        c2n.bezierCurveTo(v.cp1x, v.cp1y, v.cp2x, v.cp2y, v.ex, v.ey);
        c2n.strokeStyle = v.c + a + ")"; c2n.lineWidth = v.w; c2n.stroke();
      });
      c2n.save(); c2n.globalAlpha = 0.07 + Math.sin(t * 0.3) * 0.025;
      for (let gx = 14; gx < w; gx += 28)
        for (let gy = 12; gy < h; gy += 24) {
          c2n.beginPath();
          for (let k = 0; k < 6; k++) {
            const a2 = k * Math.PI / 3;
            k === 0 ? c2n.moveTo(gx + Math.cos(a2) * 7, gy + Math.sin(a2) * 7)
                    : c2n.lineTo(gx + Math.cos(a2) * 7, gy + Math.sin(a2) * 7);
          }
          c2n.closePath(); c2n.strokeStyle = "rgba(110,240,155,.9)"; c2n.lineWidth = 0.35; c2n.stroke();
        }
      c2n.restore();
      nodesRef.current.forEach(n => {
        const a = 0.35 + Math.sin(t * n.sp + n.ph) * 0.5;
        const g = c2n.createRadialGradient(n.x, n.y, 0, n.x, n.y, n.r * 4);
        g.addColorStop(0, n.c + a + ")"); g.addColorStop(1, n.c + "0)");
        c2n.beginPath(); c2n.arc(n.x, n.y, n.r * 4, 0, Math.PI * 2); c2n.fillStyle = g; c2n.fill();
        c2n.beginPath(); c2n.arc(n.x, n.y, n.r, 0, Math.PI * 2);
        c2n.fillStyle = n.c + Math.min(1, a + 0.15) + ")"; c2n.fill();
      });
      const lx = (st.tx * 0.5 + 0.5) * w; const ly = (st.ty * 0.5 + 0.5) * h;
      const lg = c2n.createRadialGradient(lx, ly, 0, lx, ly, w * 0.55);
      lg.addColorStop(0, "rgba(65,210,110,.28)"); lg.addColorStop(1, "transparent");
      c2n.fillStyle = lg; c2n.fillRect(0, 0, w, h);
    }

    function drawFlash(now: number) {
      if (!fc || !flashCvs || !st.flashActive) return;
      const fcn = fc; const fcv = flashCvs;
      const elapsed = now - st.flashStart;
      if (elapsed > FLASH_DUR) {
        st.flashActive = false; fcn.clearRect(0, 0, fcv.width, fcv.height); return;
      }
      const p = elapsed / FLASH_DUR; const w = fcv.width; const h = fcv.height;
      fcn.clearRect(0, 0, w, h);
      let env = p < 0.18 ? p / 0.18 : p < 0.55 ? 1 : 1 - (p - 0.55) / 0.45;
      env = Math.max(0, Math.min(1, env));
      const { flashCx: cx, flashCy: cy } = st;
      const diag = Math.sqrt(w * w + h * h);
      const ep = Math.pow(p, 0.55);
      const t2 = elapsed / 1000 * 0.8;
      FLASH_LAYERS.forEach((l, i) => {
        const nx = noise(i * 1.3, 0, t2) * 28 * (1 - p * 0.6);
        const ny = noise(0, i * 1.1, t2 + 1.5) * 22 * (1 - p * 0.6);
        const r = diag * l.r * ep;
        const g = fcn.createRadialGradient(cx + nx, cy + ny, 0, cx + nx, cy + ny, r);
        const op = l.op * env * (1 - p * 0.15);
        g.addColorStop(0, `rgba(${l.c},${Math.min(0.95, op)})`);
        g.addColorStop(0.35, `rgba(${l.c},${op * 0.6})`);
        g.addColorStop(0.7,  `rgba(${l.c},${op * 0.18})`);
        g.addColorStop(1,    `rgba(${l.c},0)`);
        fcn.globalCompositeOperation = "lighter";
        fcn.fillStyle = g; fcn.beginPath(); fcn.arc(cx + nx, cy + ny, r, 0, Math.PI * 2); fcn.fill();
      });
      const hg = fcn.createRadialGradient(cx, cy, 0, cx, cy, diag * 0.18 * ep);
      const hop = 0.7 * env * (1 - p * 0.3);
      hg.addColorStop(0, `rgba(255,230,120,${Math.min(0.9, hop)})`);
      hg.addColorStop(0.5, `rgba(240,200,60,${hop * 0.4})`);
      hg.addColorStop(1, "rgba(200,150,10,0)");
      fcn.fillStyle = hg; fcn.beginPath(); fcn.arc(cx, cy, diag * 0.18 * ep, 0, Math.PI * 2); fcn.fill();
      fcn.globalCompositeOperation = "source-over";
    }

    function animMedal(t: number) {
      const mc = root.querySelector("#gv-mc") as SVGCircleElement | null;
      const ms = root.querySelector("#gv-mstar") as SVGGElement | null;
      if (mc) mc.setAttribute("r", String((3 + Math.sin(t * 1.8) * 1.2).toFixed(2)));
      if (ms) ms.setAttribute("transform", `translate(130,170) rotate(${(t * 8).toFixed(1)})`);
    }

    function animFS(t: number) {
      for (let i = 0; i < 6; i++) {
        const el = root.querySelector(`#gv-fs${i}`) as SVGCircleElement | null;
        if (!el) continue;
        const ph = t * 2.2 + i * 0.9;
        el.setAttribute("opacity", (0.2 + Math.sin(ph) * 0.65).toFixed(2));
        el.setAttribute("transform", `translate(${(Math.sin(ph * 0.7) * 4).toFixed(1)},${(Math.cos(ph * 0.95) * 4).toFixed(1)})`);
      }
    }
    ...
    function animate(now: number) {
      st.tx = lerp(st.tx, st.mx, 0.055);
      st.ty = lerp(st.ty, st.my, 0.055);
      const oa = st.isOpen ? -34 : 0;
      const wrap = wrapRef.current;
      if (wrap)
        wrap.style.transform = `rotateY(${(-14 + st.tx * 28 + oa).toFixed(2)}deg) rotateX(${(4 - st.ty * 14).toFixed(2)}deg) rotateZ(${(st.tx * 1.8).toFixed(2)}deg)`;
      const t = now / 1000;
    ...
    const onTouchMove  = (e: TouchEvent) => {
      e.preventDefault();
      const r = root.getBoundingClientRect(); const touch = e.touches[0];
      st.mx = (touch.clientX - r.left) / r.width * 2 - 1;
      st.my = (touch.clientY - r.top)  / r.height * 2 - 1;
    };
  ...
  return (
    <div
      ...
      style={{ position: "relative", width: "100%", height: "100%", minHeight: 400, overflow: "hidden", cursor: "none", background: "transparent" }}
      aria-label="Grimoire Vivant Dédicalivres — double-clic pour ouvrir"
    >
      {/* Flash canvas — z-index maximal, hors du contexte 3D */}
      <canvas
        ref={flashCvsRef}
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%", pointerEvents: "none", zIndex: 100 }}
      />

      {/* Scène 3D — perspective isolée */}
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", perspective: "1100px", zIndex: 2 }}>
        <div ref={wrapRef} style={{ transformStyle: "preserve-3d", position: "relative", width: 260, height: 340 }}>
          <div style={{ width: 260, height: 340, transformStyle: "preserve-3d", position: "relative" }}>
            ...
            >
              {/* Face avant — SVG + canvas par-dessus (pas de foreignObject) */}
              <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden" } as React.CSSProperties}>
                <svg width="260" height="340" viewBox="0 0 260 340" xmlns="http://www.w3.org/2000/svg" style={{ position: "absolute", inset: 0 }}>
                  <defs>
                ...
                {/* Canvas animé par-dessus le SVG — SANS foreignObject */}
                <canvas
                  ref={coverCvsRef}
                  style={{ position: "absolute", inset: 0, width: 260, height: 340, pointerEvents: "none", mixBlendMode: "screen", opacity: 0.85 }}
                />
              </div>
              ...
              <svg style={{ position: "absolute", inset: 0, transform: "rotateY(180deg)", backfaceVisibility: "hidden" } as React.CSSProperties} width="260" height="340" viewBox="0 0 260 340" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  ...
                  <clipPath id="gv-b4s"><path d="M8,3 C5,1 2,2 1,5 L0,14 L0,320 C0,328 1,334 4,337 L8,340 L252,339 C256,339 259,337 260,333 L260,30 C260,18 258,8 254,4 L248,1 Z"/></clipPath>
                </defs>
                <g clipPath="url(#gv-b4s)">
                  ...
                  <text x="130" y="152" textAnchor="middle" fontFamily="'Dancing Script',cursive" fontSize="22" fontWeight="600" fill="rgba(240,220,155,0.92)">rencontres</text>
                  <text x="130" y="185" textAnchor="middle" fontFamily="'Dancing Script',cursive" fontSize="18" fontWeight="600" fill="rgba(235,215,150,0.85)">de demain.</text>
